...
export default function MasterLandingPage() {
  ...
  // Hydration sync: Safe client-side execution
  useEffect(() => {
    setMounted(true);
  }, []);

  // Terminal Typer Effect
  useEffect(() => {
    if (!mounted) return;
  ...
  return (
    <div className="min-h-[100dvh] bg-[#050505] text-white relative flex flex-col font-sans selection:bg-blue-500/30 overflow-hidden antialiased">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute top-1/2 left-[-10%] w-[50vw] h-[50vw] bg-blue-600/10 rounded-full blur-[150px] mix-blend-screen"></div>
        <div className="absolute bottom-[-10%] right-[-10%] w-[40vw] h-[40vw] bg-emerald-600/10 rounded-full blur-[120px] mix-blend-screen"></div>
      </div>
      ...
      <main className="flex-1 relative z-10 w-full max-w-[1400px] mx-auto px-6 py-12 md:py-20 flex flex-col lg:flex-row items-center justify-center gap-16 lg:gap-24">
        ...
        <div className="flex-1 max-w-2xl w-full">
          <div className="flex flex-wrap items-center gap-3 mb-8">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-neutral-900 border border-neutral-800 rounded-full">
              ...
              <span className="text-[10px] font-mono font-bold text-neutral-400 uppercase tracking-widest">Base L2 Mainnet Live</span>
            </div>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-blue-900/20 border border-blue-500/20 rounded-full">
              <span className="text-[10px] font-mono font-bold text-blue-400 uppercase tracking-widest">Utility Settlements Relayer</span>
            </div>
          </div>

          <h1 className="text-5xl md:text-6xl lg:text-7xl font-extrabold tracking-tighter leading-[1.05] text-white mb-8">
            Decentralized <br />
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-400 via-blue-200 to-white">Physical</span> <br className="hidden md:block" />
            Infrastructure.
          </h1>

          <p className="text-sm md:text-base text-neutral-400 font-mono leading-relaxed mb-10 max-w-xl">
            An elite protocol engine for residential management. True 0-click USDC relayer execution, infinite DOM virtualization, and cryptographic invariant logging.
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <button 
              ...
              className="w-full sm:w-auto bg-blue-600 hover:bg-blue-500 text-white px-8 py-4 rounded-xl text-[11px] font-mono font-bold uppercase tracking-widest transition-all shadow-[0_0_30px_rgba(37,99,235,0.3)] flex items-center justify-center gap-3 disabled:opacity-50"
            >
              {isInitializing ? (
        ...
        <div className="flex-1 w-full max-w-2xl relative">
          ...
          <div className="bg-[#0A0A0A] border border-white/10 rounded-2xl overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.8)] relative z-10 aspect-[4/3] flex flex-col">
            <div className="bg-neutral-900 border-b border-white/[0.05] px-4 py-3 flex items-center gap-3">
              <div className="flex gap-2">
                <div className="w-3 h-3 rounded-full bg-neutral-700"></div>
                <div className="w-3 h-3 rounded-full bg-neutral-700"></div>
                <div className="w-3 h-3 rounded-full bg-neutral-700"></div>
              </div>
              <span className="text-[10px] font-mono text-neutral-500 uppercase tracking-widest ml-2">CompoundOS // TTY1</span>
            </div>
            ...
            <div className="p-6 font-mono text-[11px] md:text-xs leading-relaxed flex-1 overflow-hidden relative">
               <div className="absolute top-6 left-6 right-6 bottom-6 overflow-y-auto scrollbar-none flex flex-col justify-end">
                 {/* Structural Parity: Only render logs if mounted, preventing SSR crash */}
                 {mounted && logs.map((log, index) => (
                   <p key={index} className={`mb-2 animate-in fade-in duration-300 ${log.includes('[OK]') ? 'text-emerald-400' : log.includes('OPTIMAL') ? 'text-blue-400 font-bold' : 'text-neutral-400'}`}>
                     {log}
                   </p>
                 ))}
                 
                 {mounted && logs.length < bootSequence.length && (
