  ...
  return (
    <div className="min-h-[100dvh] flex flex-col md:flex-row bg-[#050505] text-white relative overflow-hidden font-sans antialiased">
      <div className="absolute inset-0 pointer-events-none z-0">
        ...
        <div className="absolute inset-0 opacity-[0.03]" style={{ backgroundImage: `radial-gradient(circle at 2px 2px, rgba(255,255,255,1) 1px, transparent 0)`, backgroundSize: '32px 32px' }}></div>
      </div>
          ...
          <div className="space-y-6">
            <div className="space-y-3">
              <button
                ...
                disabled={isLoading}
                className="w-full flex items-center justify-center gap-3 bg-white hover:bg-neutral-200 text-black py-4 rounded-xl text-[10px] md:text-[11px] font-mono font-bold uppercase tracking-widest transition-colors disabled:opacity-50 shadow-[0_0_20px_rgba(255,255,255,0.05)]"
              >
              ...
              {/* FIXED WEB3 WALLET BUTTON LAYOUT */}
                ...
                {mounted ? (
                  <ConnectButton.Custom>
                    {({ account, chain, openAccountModal, openChainModal, openConnectModal, mounted: rkMounted }) => {
                      ...
                      if (!ready) {
                        return <div className="w-full py-4 rounded-xl bg-white/5 animate-pulse h-[46px]"></div>;
                      }

                      if (!connected) {
                        return (
                          <button onClick={openConnectModal} type="button" className="w-full flex items-center justify-center gap-3 bg-[#111] hover:bg-[#222] text-white py-4 rounded-xl text-[10px] md:text-[11px] font-mono font-bold uppercase tracking-widest transition-colors border border-white/10 shadow-[inset_0_0_10px_rgba(255,255,255,0.02)]">
                      ...
                      if (chain.unsupported) {
                        return (
                          <button onClick={openChainModal} type="button" className="w-full flex items-center justify-center gap-3 bg-red-900/30 hover:bg-red-900/50 text-red-400 border border-red-500/30 py-4 rounded-xl text-[10px] md:text-[11px] font-mono font-bold uppercase tracking-widest transition-colors">
                      ...
                      return (
                        <div className="flex items-center gap-2 w-full animate-in fade-in duration-300">
                          ...
                          {/* EXPANDING WALLET ADDRESS PILL (RIGHT) */}
                          <button 
                  ...
                  <div className="w-full py-4 rounded-xl bg-white/5 animate-pulse h-[46px]"></div>
                )}
              </div>
            </div>
            ...
            <form onSubmit={handleEmailAuth} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-[#050505] border border-white/10 px-4 py-4 rounded-xl font-mono text-[11px] text-white placeholder-neutral-700 focus:outline-none focus:border-blue-500/50 transition-colors shadow-inner"
                />
              </div>
